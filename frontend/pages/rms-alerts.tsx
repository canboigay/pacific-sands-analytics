...
export default function RMSAlertsAndSuggestions() {
  ...
  const checkForAlerts = () => {
    ...
    if (daysOut > 30 && metrics.occupancy < 0.70) {
      newAlerts.push({
        priority: 'HIGH',
        type: 'OCCUPANCY',
    ...
    } else if (daysOut <= 7 && metrics.occupancy < 0.60) {
      newAlerts.push({
        priority: 'CRITICAL',
        type: 'OCCUPANCY',
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
                  ...
                  <div key={idx} style={{
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                        ...
                        <div>
                          <div style={{ fontWeight: 'bold', fontSize: '1.15rem', color: '#333' }}>
                            {alert.title}
                          </div>
                      ...
                      <div>
                        ...
                        <ul style={{ margin: '0 0 0 20px', color: '#666' }}>
                          {suggestion.tactics.map((tactic, i) => (
                            <li key={i} style={{ marginBottom: '4px' }}>{tactic}</li>
                          ))}
                        </ul>
                      </div>
                    )}
        ...
        <div style={{
          ...
          <div style={{
            ...
            {[
              { label: 'Occupancy', value: `${(currentMetrics.occupancy * 100).toFixed(1)}%`, target: '85-90%' },
              { label: 'ADR', value: `$${currentMetrics.averageRate.toFixed(0)}`, target: '+1-3% YoY' },
              { label: 'RevPAR', value: `$${currentMetrics.revpar.toFixed(0)}`, target: 'Maximize' },
              { label: 'Pickup ADR', value: `$${currentMetrics.pickupADR.toFixed(0)}`, target: '+$100-150' }
            ].map((metric, idx) => (
              <div key={idx} style={{
                background: '#f8f9fa',
                borderRadius: '10px',
